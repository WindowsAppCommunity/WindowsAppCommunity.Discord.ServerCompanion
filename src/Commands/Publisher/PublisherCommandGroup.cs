...
[Group("publisher")]
public class PublisherCommandGroup(ICoreApi client, IKuboOptions kuboOptions, IWacNomadRepoGroupRepository nomadRepoService, IInteractionContext interactionContext, IFeedbackService feedbackService, IDiscordRestInteractionAPI interactionAPI, IDiscordRestChannelAPI channelApi, IDiscordRestGuildAPI guildApi, ICommandContext context) : Remora.Commands.Groups.CommandGroup
{
    [Command("create")]
    public async Task<IResult> CreatePublisherAsync(string repoId, string name, string description)
    {
        ...
        var createdPublisher = await nomadRepoItem.RepositoryGroup.PublisherRepository.CreateAsync(new(KnownId: knownId), CancellationToken.None);
    ...
    public async Task<IResult> GetPublisherAsync(string publisherId)
    {
        ...
        var publisher = await nomadRepoItem.RepositoryGroup.PublisherRepository.GetAsync(publisherId, CancellationToken.None);
    ...
    public async Task<IResult> ListPublisherAsync()
    {
        ...
        var responseBuilder = new StringBuilder();
        await foreach (var publisher in nomadRepoItem.RepositoryGroup.PublisherRepository.GetAsync(CancellationToken.None))
        {
